...
// Custom shader material for the fluid/liquid orb effect
const vertexShader = `
  uniform float uTime;
  ...
  void main() {
    ...
    // Create displacement
    vec3 pos = position;
    float noise = snoise(pos * uDistortionFrequency + uTime * 0.3);
    float displacement = noise * uDistortionStrength;
...
const fragmentShader = `
  ...
  void main() {
    // Create gradient based on position and displacement
    float gradient = (vPosition.y + 1.5) / 3.0;
    gradient = clamp(gradient, 0.0, 1.0);
    
    // Mix colors based on gradient and displacement
    vec3 color = mix(uColor1, uColor2, gradient);
    color = mix(color, uColor3, vDisplacement * 2.0 + 0.5);
    
    // Add fresnel effect for glassy look
    ...
    float iridescence = sin(vDisplacement * 10.0 + uTime) * 0.1 + 0.1;
    color += vec3(iridescence * 0.2, iridescence * 0.1, iridescence * 0.3);
